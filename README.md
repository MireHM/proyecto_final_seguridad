...
### 1. Demostración de CONFIDENCIALIDAD
- Mostrar cómo las contraseñas se almacenan hasheadas en la BD
- Demostrar que sin token JWT no se puede acceder a recursos
- Mostrar headers de seguridad con Helmet

### 2. Demostración de INTEGRIDAD
- Intentar ingresar datos inválidos (números negativos, texto muy largo)
- Mostrar mensajes de validación
- Ver constraints en la base de datos

### 3. Demostración de DISPONIBILIDAD
- Mostrar logs en `logs/application.log`
- Mostrar tabla de auditoría con todas las acciones
- Demostrar rate limiting (hacer muchas peticiones rápidas)

### 4. Demostración de MFA
- Configurar MFA escaneando QR
- Login con código de 6 dígitos
- Mostrar rechazo de códigos inválidos

## 📸 Capturas Sugeridas

1. Pantalla de login mostrando las características de seguridad
2. Configuración de MFA con código QR
3. Panel de inventario funcionando
4. Código de hash de contraseñas
5. Código de validación de datos
6. Logs de auditoría
7. Tabla de audit_log en la base de datos
8. Verificación exitosa de MFA

## 🛠️ Tecnologías Utilizadas

- **Backend:** Node.js + Express
- **Base de Datos:** SQLite3
- **Seguridad:** bcrypt, JWT, Helmet, express-validator
...
- El sistema genera automáticamente un usuario `admin` en el primer inicio
- Los logs se guardan en la carpeta `logs/`
- La base de datos se crea automáticamente en `database.sqlite`
- El secreto JWT está hardcodeado para demostración, en producción debe estar en variables de entorno

## 🎓 Controles de Seguridad Implementados

| Control | Implementación | Ubicación |
|---------|----------------|-----------|
| Encriptación de contraseñas | bcrypt | utils/database.js |
| Autenticación JWT | jsonwebtoken | utils/auth.js |
| MFA/2FA | TOTP con Speakeasy | routes/auth.js |
| Validación de inputs | express-validator | routes/*.js |
| Rate limiting | express-rate-limit | server.js |
| Headers de seguridad | Helmet | server.js |
| Logging | Winston | utils/logger.js |
| Auditoría | Tabla audit_log | utils/database.js |
